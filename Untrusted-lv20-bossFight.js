...
function startLevel(map) {
    ...
    // INSERT THESE LINES
    map.placeObject(map.getWidth()/2,map.getHeight()-4, 'block');
    var boss = map.getDynamicObjects();
    map.getPlayer().setPhoneCallback(function(){
        for (i=0;i<=23;i++)
            map.placeObject(boss[i].getX(),boss[i].getY()-1,'bullet');
    });
    // END
    /* How to win:
    When the bosses are on the right, hide behind the created block
    When the bosses move to the left, go pick the phone
    Press [Q] until all the bosses are destroyed, theAlgorithm will then appear*/
